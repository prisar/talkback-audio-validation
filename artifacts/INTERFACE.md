...
| `program` | string | `Universal`, `Noisy Environment`, `Restaurant`, `Music` | `Universal` |
...
2. **Values containing spaces need double quoting**: `--es program "'Noisy Environment'"`. With single quoting only, the local shell splits the value and the app receives `Noisy`.
...
| `program_name` | `Noisy Environment` | `Program, Noisy Environment, selected` |
